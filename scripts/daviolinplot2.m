...
%% ADAPTED BY DORUKHAN AÇIL
% to limit outliers to 3 SD +/- Mean which wasn't
...
addOptional(p, 'outlier_type', 'sd3'); 
...
for g = 1:num_groups 
    ...
    for k = 1:num_locs
        ...
        if confs.box~=0
            ...
            std_dev = std(data_vals);

            % draw whiskers
            if confs.whiskers==1
                
                if strcmp(confs.outlier_type, 'sd3')
                    ol = data_vals < (mean_data - 3 * std_dev);  % Lower outliers (3 SD)
                    ou = data_vals > (mean_data + 3 * std_dev);  % Upper outliers (3 SD)
                else
                    ol = data_vals < (pt(3,k) - confs.outfactor * IQR(k));  % Lower outliers (IQR)
                    ou = data_vals > (pt(5,k) + confs.outfactor * IQR(k));  % Upper outliers (IQR) 
                end
        ...
        if confs.outliers==1    
            if strcmp(confs.outlier_type, 'sd3')
                ox = data_vals < (mean_data - 3 * std_dev) | ...
                data_vals > (mean_data + 3 * std_dev);
            else     
